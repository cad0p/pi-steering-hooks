...
- [`pi-steering-commit-format`](../pi-steering-commit-format) — a sibling package in this monorepo. Ships the `commitFormat` predicate plus a `commitFormatFactory` for composing custom format checkers; bundled formats include Conventional Commits (Angular preset) and bracketed JIRA-style references.
