...
| `effectiveCwd(script, initialCwd)` | fn | Compute a `Map<CommandRef, string>` of cwd-as-the-command-starts for every command in the script. Honors subshell isolation, brace-group propagation, and pipeline per-peer subshells; resolves `cd ABS`, `cd REL`, `cd ~`, `cd ~/x`, `cd` (no args) and treats `cd -` as a no-op. |
